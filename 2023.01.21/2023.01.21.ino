...
void keep_line_going(void) // 라인에 따른 주행
{
  ...
  switch(re)
  {
    case -5 : motor_control_left(1,100);; break;
    case -4 : motor_control_left(1,100);; break;
    case -3 : motor_control(1,0,72*2); break;
    case -2 : motor_control(1,15*2,72*2); break;     
    case -1 : motor_control(1,30*2,72*2); break;
    case 10 : motor_control(1,60*2,72*2); break;
    case 100 : motor_control(1,60*2,72*2); break;
    case 1 : motor_control(1,60*2,36*2); break;
    case 2 : motor_control(1,60*2,18*2); break;
    case 3 : motor_control(1,60*2,0); break;
    case 4 : motor_control_right(1,100); break;     
    case 5 : motor_control_right(1,100); break;
...
void keep_sensor_going(void) // 초음파 거리에 따른 주행
{
  ...
  if(re == 100)
  {
    if(R_Sonar_distance >= 400 && R_Sonar_distance <= 1000) // 좌 우 막힘
    {
      if(L_Sonar_distance >= 400 && L_Sonar_distance <= 1000)
      {
        motor_control(1,60*2,72*2);
      }
    }

    if(L_Sonar_distance >= 250 && L_Sonar_distance <= 1000) // 좌 앞 막힘(우회전)
    {

      if(F_Sonar_distance < 600 && F_Sonar_distance > 0) // 추정 깔끔한 우회전
      { 
        motor_control(1,60*,36*2);
      }
      
      else if(F_Sonar_distance < 200 && F_Sonar_distance > 0)
      {
        motor_control(1,60*2,0);
      }
    }
    
    if (R_Sonar_distance < 400 && R_Sonar_distance > 0) // 오른쪽 막힘 (좌회전)
    {
      motor_control(1,50*2,72*2);
    }

    else if (R_Sonar_distance < 200 && R_Sonar_distance > 0)
    {
      motor_control(1,30*2,72*2);
    }
    
    if (L_Sonar_distance < 250 && L_Sonar_distance > 0)// 왼쪽막힘 (우회전)
    {
      motor_control(1,60*2,62*2);
    }

    else if (R_Sonar_distance < 200 && R_Sonar_distance > 0)
    {
      motor_control(1,60*2,36*2);
    }
