...
void keep_sensor_going(void) // 초음파 거리에 따른 주행
{
  ...
  if(re == 100)
  {
    if(R_Sonar_distance - 10 <= L_Sonar_distance && R_Sonar_distance + 10 >= L_Sonar_distance) // 좌 우 거리 같을때 진행
    {
      motor_control(1,60*2,72*2);
    }

    if(L_Sonar_distance < 1050) // 추정 깔끔한 우회전
    {

      if(F_Sonar_distance < 600 && F_Sonar_distance > 200) // 좀 멀때
      { 
        motor_control(1,60*2,36*2);
        delay(1000);
      }

      if(F_Sonar_distance < 200) // 좀 가까울때
      {
    ...
    if (L_Sonar_distance + 5 > R_Sonar_distance) // 오른쪽이 더 가까움 (좌회전)
    {
      motor_control(1,50*2,72*2);
    }

    
    if (L_Sonar_distance < R_Sonar_distance + 5) // 왼쪽이 더 가까움 (우회전)
    {
