...
void keep_line_going(void) // 라인에 따른 주행
{
  ...
  switch(re)
  {
    ...
    case -1 : motor_control(1,42*2,100*2); break;
    case 10 : motor_control(1,84*2,100*2); break;
    case 100 : motor_control(1,84*2,100*2); break;
    case 1 : motor_control(1,84*2,50*2); break;
...
void keep_sensor_going(void) // 초음파 거리에 따른 주행
{
  ...
  if(re == 100)
  {
    ...
    if(F_Sonar_distance <= 800 && R_Sonar_distance > 2000) // 미로 우회전
    {
    ...
    {
      if(R_Sonar_distance + L_Sonar_distance <= 1170 && R_Sonar_distance - 30 <= L_Sonar_distance && R_Sonar_distance + 30 >= L_Sonar_distance) // 좌 우 거리 같을때 진행
      {
        motor_control(1,84*2,100*2);
