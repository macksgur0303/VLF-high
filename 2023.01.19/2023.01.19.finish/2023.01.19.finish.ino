...
void keep_sensor_going(void) // 초음파 거리에 따른 주행
{
  ...
  if(re == 100)
  {
    if(R_Sonar_distance >= 250 && R_Sonar_distance <= 600) // 좌 우 막힘
    {
      if(L_Sonar_distance >= 250 && L_Sonar_distance <= 600)
      {
    ...
    {
      if(F_Sonar_distance < 450)
      {
        if(F_Sonar_distance > 0)
        {
          motor_control(2,60*2,72*2);
          delay(500);
        }
      }
    }
    if (R_Sonar_distance < 250) // 오른쪽 막힘 (좌회전)
    {
    ...
    if (L_Sonar_distance < 250)// 왼쪽막힘 (우회전)
    {
